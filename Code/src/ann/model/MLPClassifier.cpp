...
double_tensor MLPClassifier::predict(DataLoader<double, double>* pLoader,
  ...
  int total_batch = 300;
  xt::xarray<double> results = xt::zeros<double>({total_batch, 3});
  size_t row_index = 0;
  for (auto batch : *pLoader) {
    // TODO YOUR CODE IS HERE
    double_tensor X = batch.getData();
    double_tensor Y = forward(X);
    for (size_t i = 0; i < Y.shape()[0];
         ++i) {  // Y.shape()[0] is the number of rows in Y
      results(row_index, 0) = Y(i, 0);
      results(row_index, 1) = Y(i, 1);
      results(row_index, 2) = Y(i, 2);
      row_index++;  // Move to the next row in results
    }
  }
...
double_tensor MLPClassifier::forward(double_tensor X) {
  ...
  cout << endl;
  cout << endl;
  cout << shape2str(output.shape()) << endl;
  for (auto layer : m_layers) {
    output = layer->forward(output);
    cout << layer->get_desc() << endl;
    cout << shape2str(output.shape()) << endl;
  }
  cout << endl;
  cout << endl;
