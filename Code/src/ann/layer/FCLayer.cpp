...
xt::xarray<double> FCLayer::forward(xt::xarray<double> X) {
  ...
  // Compute the output Y = X * W^T
  xt::xarray<double> Y = xt::linalg::dot(X, xt::transpose(m_aWeights));

  // If bias is used, add it to each row of Y
  if (m_bUse_Bias) {
    Y += m_aBias;
  }
  return Y;
...
xt::xarray<double> FCLayer::backward(xt::xarray<double> DY) {
  ...
  for (std::size_t i = 0; i < DY.shape()[0]; ++i) {
    xt::view(dW_batch, i) = xt::linalg::outer(
        xt::view(DY, i), xt::transpose(xt::view(m_aCached_X, i)));
  }
  m_aGrad_W = xt::sum(dW_batch, {0});

  xt::xarray<double> delta_X = xt::linalg::dot(DY, m_aWeights);
